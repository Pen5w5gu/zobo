...
public interface BookingDAO extends JpaRepository<Bookings, Integer> {
	...
	@Query("SELECT new duan.sportify.DTO.BookingDTO(" +
			"b.bookingid, " +
			"b.username, " +
			"b.bookingdate, " +
			"b.bookingprice," +
			...
			"JOIN Shifts s ON s.shiftid = bd.shiftid " +
			"JOIN Field f ON f.fieldid = bd.fieldid " +
			"JOIN Users  u ON u.username = b.username " +
			"WHERE f.field_owner = :username")
	List<BookingDTO> findBookingsByFieldOwner(String username);
	@Query("SELECT COUNT(b) FROM Bookingdetails b WHERE b.playdate = CURRENT_DATE AND b.field.field_owner = :username")
	int countBookingsToday(@Param("username") String username);

	@Query("SELECT COALESCE(SUM(b.price), 0) FROM Bookingdetails b WHERE b.playdate = CURRENT_DATE AND b.field.field_owner = :username")
	double sumRevenueToday(@Param("username") String username);

	@Query("SELECT new duan.sportify.DTO.BookingDetailDTO(" +
			"f.fieldid, f.namefield, s.shiftid, s.nameshift, s.starttime, s.endtime, " +
			"CASE WHEN bd.bookingdetailid IS NOT NULL THEN 'Đã đặt' ELSE 'Chưa đặt' END) " +
			"FROM Field f " +
			"JOIN Shifts s ON 1=1 " +
			"LEFT JOIN Bookingdetails bd ON f.fieldid = bd.field.fieldid " +
			"AND s.shiftid = bd.shifts.shiftid " +
			"AND bd.playdate = :currentDate " +
			"WHERE f.field_owner = :username " +
			"ORDER BY f.fieldid, s.shiftid")
	List<BookingDetailDTO> getSlotStatusForOwner(@Param("username") String username, @Param("currentDate") java.util.Date currentDate);
